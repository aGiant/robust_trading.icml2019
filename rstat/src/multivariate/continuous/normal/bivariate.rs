...
use spaces::{continuous::Reals, product::DoubleSpace, Matrix, Vector};
use std::fmt;
...
impl Distribution for BivariateNormal {
    type Support = DoubleSpace<Reals>;

    fn support(&self) -> DoubleSpace<Reals> {
...
impl MultivariateMoments for BivariateNormal {
    fn mean(&self) -> Vector<f64> {
        Vector::from_vec(self.mu.to_vec())
    }
